...
if [[ -f $DOTNET_TAR && -f $NETCORE_PLUGINS_TAR ]]; then
...
echo "Started in \$(date +"%Y_%m_%d-%H:%M:%S")" > log/start_log.txt
if [ -e $MPLC_PATH/prerun.sh ]; then 
	. $MPLC_PATH/prerun.sh 1>> log/start_log.txt 2>&1
fi
enable_dump(){
	ulimit -c unlimited
	sysctl kernel.core_pattern=\$PWD/log/%e.core 1>> log/start_log.txt 2>&1
}
$USE_DUMP
...
start(){
	local PORT=\$1
	local PARAMS=\$2
	echo "Start mplc on \$PORT" >> log/start_log.txt
	local LOG_OPT=""
	if [ "\$MPLC_LOGS" == ON ]; then
		LOG_OPT="/log:log/mplc_\$CURDATE.txt"
	fi
	$MPLC_PATH/mplc_service \$PORT $MPLC_PATH/mplc new \$INST_OPTS \$LOG_OPT \$START_OPTS \$PARAMS 1>> log/start_log.txt 2>&1
}
start_examplar(){
	local BASE_PORT=\$1
	local EXEMPLAR=\$2
	...
	echo "Start mplc examplar \$EXEMPLAR on \$PORT" >> log/start_log.txt
	local LOG_OPT=""
	if [ "\$MPLC_LOGS" == ON ]; then
		LOG_OPT="/log:log/\$EXEMPLAR/mplc_\$CURDATE.txt"
	fi
	$MPLC_PATH/mplc_service \$PORT $MPLC_PATH/mplc new \$INST_OPTS \$LOG_OPT \$START_OPTS "/ea:\$EXEMPLAR" 1>> log/\$EXEMPLAR/start_log.txt 2>&1
}
run_with_check(){
	local NAME=\$1
	local CMD=\$2
...
run_report_builder(){
	...
	local LOG_OPT=""
	if [ "\$MPLC_LOGS" == ON ]; then
		LOG_OPT="--log log/node_\$CURDATE.log"
	fi
	$NODEJS_DIR/node_ms4d $NODEJS_DIR/index.js --mplc $MPLC_URL --exemplars \$COUNT $NODEJS_OPTS \$LOG_OPT &
}
...
install_netcore() {
	...
	mplc_rm  $DOTNET_DIR
	mplc_unzip $DOTNET_TAR $MPLC_PATH 1>/dev/null
	[ "$MPLC_USER" == "root" ] ||
		chown $MPLC_USER:$MPLC_GROUP -R $DOTNET_DIR
	echo OK
	
	echo -n "Install netcore plugins...       "
	mplc_unzip $NETCORE_PLUGINS_TAR $MPLC_PATH 1>/dev/null
	[ "$MPLC_USER" == "root" ] ||
		chown $MPLC_USER:$MPLC_GROUP -R $NETCORE_PLUGINS_DIR
	echo OK
